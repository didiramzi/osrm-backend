    ...
    : public osmium::relations::Collector<ConditionalRestrictionsCollector, true, true, false>
{
  public:
    ...
    ConditionalRestrictionsCollector(action_t action)
        : tag_filter(false), action(action)
    {
...
int main(int argc, char *argv[])
{
    ...
    ConditionalRestrictionsCollector collector(
        [&get_local_time_at_point](const osmium::Way &from, const osmium::Node &via, const osmium::Way &to, const char *condition) {
            ...
            const auto& opening_hours = osrm::extractor::parseOpeningHours(condition);
            if (!osrm::extractor::checkOpeningHours(opening_hours, local_time))
                return;
